...
public:
	...
	folder.*/
	Oscil(const int * TABLE_NAME):table(TABLE_NAME)
	{}
	...
	*/
	void setTable(const int * TABLE_NAME)
	{
		table = TABLE_NAME;
	}
	...
	*/
	...
	{
		incrementPhase();
		return (int)pgm_read_word_near(table + (((phase_fractional+(phmod_proportion * NUM_TABLE_CELLS))>>OSCIL_F_BITS) & (NUM_TABLE_CELLS - 1)));
	}
...
*/
	...
	*/
	...
	{
		return (int)pgm_read_word_near(table + (index & (NUM_TABLE_CELLS - 1)));
	}
...
static const unsigned char ADJUST_FOR_NUM_TABLE_CELLS = (NUM_TABLE_CELLS<2048) ? 8 : 0;
	...
	{
#ifdef OSCIL_DITHER_PHASE
		return (int)pgm_read_word_near(table + (((phase_fractional + ((int)(xorshift96()>>16))) >> OSCIL_F_BITS) & (NUM_TABLE_CELLS - 1)));
#else
		return (int)pgm_read_word_near(table + ((phase_fractional >> OSCIL_F_BITS) & (NUM_TABLE_CELLS - 1)));
